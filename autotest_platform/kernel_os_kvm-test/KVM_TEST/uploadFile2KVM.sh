...
cmdStr="Now the file:testRemoteSh.sh will be uploaded to the KVM:$kvmName ..."
echo $cmdStr
write_log "INFO" "${cmdStr}"

#expect scp2Remote.exp $KVMIP root loongson testRemoteSh.sh /home >/dev/null 2>&1
expect scp2Remote.exp $KVMIP root loongson testRemoteSh.sh /home


cmdStr="The file:testRemoteSh uploadeded completely."
echo $cmdStr
write_log "INFO" "${cmdStr}"
