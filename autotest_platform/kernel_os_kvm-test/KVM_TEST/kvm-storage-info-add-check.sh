...
diskInputNum=0
#----------------------------------------------------------------
cmdEndStr="KVM Test:View the storage(disk) info of the KVM after add 1 disk End----------------------------"
KVMIP=''
retCode=0
#----------------------------------------------------------------------------------
...
write_log "INFO" "KVM Test: View the storage(disk) info of the KVM after add 1 disk Begin----------------------------"
echo "Check disk info of the KVM after add 1 disk Begin..."
...
if [  $diskCount -eq 1 ]; then
  cmdStr="Error:The KVM has only 1 disk info.Exit..."
  echo $cmdStr
  write_log "INFO" "${cmdStr}"
  exit 1
else
  cmdStr="the KVM has not only 1 disk info.Continue to check..."
  echo $cmdStr
...
if [  $retCode -eq 0 ]; then
  cmdStr="The KVM has vda disk,it is default disk info,continue to check other disk info..."
  echo $cmdStr
  write_log "INFO" "${cmdStr}"
else
  cmdStr="Error:The KVM has no vda disk(default disk),Please check it!"
  echo $cmdStr
  write_log "ERROR" "${cmdStr}"
  exit 1
fi
...
if [  $retCode -eq 0 ]; then
  ...
  cmdStr="Error:The KVM has no vda disk,Please check it!"
  echo $cmdStr
  write_log "ERROR" "${cmdStr}"
  exit 1
fi
...
if [  $retCode -eq 0 ]; then
  cmdStr="The KVM has vdb disk,it is a disk info after add new."
  echo $cmdStr
  write_log "INFO" "${cmdStr}"
  cmdStr="Add 1 new disk Success.."
  echo $cmdStr
  write_log "INFO" "${cmdStr}"

else
  cmdStr="Error:The KVM has no vdb disk,add a new disk failed!Please check it!"
  echo $cmdStr
  write_log "ERROR" "${cmdStr}"
  exit 1
fi
