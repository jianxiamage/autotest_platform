...
#----------------------------------------------------------------
#-------------------------------
cmdEndStr="KVM Test:Upload perform-wget.sh to KVM End----------------------------"
KVMIP=''
#----------------------------------------------------------------------------------
source ./kvm-log.sh
logFile=$logFile
...
write_log "INFO" "KVM Test:Upload perform-wget.sh to KVM Begin----------------------------"
...
cmdStr="Now the file:perform-wget.sh will be uploaded to the KVM:$kvmName ..."
echo $cmdStr
write_log "INFO" "${cmdStr}"

#expect scp2Remote.exp $KVMIP root loongson testRemoteSh.sh /home >/dev/null 2>&1
expect scp2Remote.exp $KVMIP root loongson perform-wget.sh /home


cmdStr="The file:perform-wget.sh uploadeded completely."
echo $cmdStr
write_log "INFO" "${cmdStr}"
