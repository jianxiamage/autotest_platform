...
#Begin set 4 cpus
echo "KVM cpu test:set cpu to 4(cpu config:current->1,Max:4)"
virsh setvcpus $kvmName 4

#wait 30s after cpu set
sleep 30s
...
echo "=================add cpu to 4(1to4) End===================="

#Begin to test cpu remove
echo "KVM cpu test:remove cpu to 1(4to1)"
...
retCode=$?
echo "---------set cpu number to 1,retCode:$retCode"
trap 'exit_err $LINENO $?'     ERR

if [  $retCode -ne 0 ]; then
  cmdStr="You should not remove cpu number to 1 as the setting info is 2."
  echo $cmdStr
  write_log "INFO" "${cmdStr}"
  exit 0
else
  cmdStr="Error:something wrong ocurred,Please check it!"
  echo $cmdStr
  write_log "INFO" "${cmdStr}"
  exit 1 
fi
