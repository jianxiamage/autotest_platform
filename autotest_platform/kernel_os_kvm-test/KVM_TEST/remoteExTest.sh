...
#----------------------------------------------------------------
#-------------------------------
cmdEndStr="KVM Test:Test remote Sh End----------------------------"
KVMIP=''
#----------------------------------------------------------------------------------
source ./kvm-log.sh
logFile=$logFile
...
write_log "INFO" "KVM Test: test remote Sh Begin----------------------------"
...
cmdStr="Begin to excuete sh file:testRemoteSh.sh on KVM:$kvmName ..."
echo $cmdStr
write_log "INFO" "${cmdStr}"
...
sh uploadFile2KVM.sh

cmdStr="The test:testRemoteSh.sh on KVM:$kvmName is end normally."
echo $cmdStr
write_log "INFO" "${cmdStr}"

expect sshExecCmd.exp $KVMIP root loongson "/home/testRemoteSh.sh"
