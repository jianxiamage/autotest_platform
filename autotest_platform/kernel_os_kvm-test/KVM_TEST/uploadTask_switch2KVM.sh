...
write_log "INFO" "KVM Test:Upload  task_switch.sh to KVM Begin----------------------------"
...
cmdStr="Now the file:task_switch.sh will be uploaded to the KVM:$kvmName ..."
echo $cmdStr
write_log "INFO" "${cmdStr}"

#expect scp2Remote.exp $KVMIP root loongson testRemoteSh.sh /home >/dev/null 2>&1
expect scp2Remote.exp $KVMIP root loongson task_switch.sh /home


cmdStr="The file:task_switch.sh uploadeded completely."
echo $cmdStr
write_log "INFO" "${cmdStr}"
