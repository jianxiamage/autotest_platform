...
#Init the cpu info(current:3,Max:4)
sh kvm-cpu-init-3to4.sh
echo "kvm-cpu-init-3to4.sh retCode:$?"
echo "cpu config:current->3,Max:4"

#Begin to test cpu add
echo "KVM cpu test:add 1 cpu (current:3->4)"
