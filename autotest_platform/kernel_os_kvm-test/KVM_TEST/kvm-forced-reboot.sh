...
if virsh domstate $kvmName |grep -q "关闭";then
   cmdStr="$kvmName is poweroff,you will start it to test."
   echo $cmdStr
   write_log "WARNING" "${cmdStr}"

elif virsh domstate $kvmName |grep -q "running";then
  cmdStr="$kvmName is running.Begin to test [forced reboot]..."
  echo ${cmdStr}
  write_log "INFO" "${cmdStr}"

  trap - ERR

  sh kvm-forced-reboot-opt.sh
  retCode=$?

  echo "=====kvm-forced-reboot-opt.sh retCode:$retCode"
  trap 'exit_err $LINENO $?'     ERR
  echo "==========================Test [forced reboot] Result================================"
  if [ $retCode -eq 0 ]; then
    cmdStr="Test for forced reboot success."
    echo $cmdStr
    write_log "INFO" "${cmdStr}"
    echo "==========================Test [forced reboot] Result================================"
    exit 0
  else
    cmdStr="Test for forced reboot failed.Please check it!"
    echo $cmdStr
    write_log "INFO" "${cmdStr}"
    echo "==========================Test [forced reboot] Result================================"
    exit 1
...
if virsh domstate $kvmName |grep -q "running";then
   ...
   echo "=====kvm-forced-reboot.sh retCode:$retCode"
   trap 'exit_err $LINENO $?'     ERR
